...
bool submit(int p, const DecodeRequest& r){ TcpConnection c; if(!c.connect("127.0.0.1",p).ok()) return false; if(!c.send_frame(FrameType::SubmitRequest, encode_submit_request(r)).ok()) return false; auto fr=c.recv_frame(); if(!fr.ok()||fr.value().type!=FrameType::SubmitAck) return false; auto a=decode_submit_ack(fr.value().payload); return a.ok()&&a.value().admitted; }
...
DF_TEST(multiprocess_atomic_closure){
  ...
  bool any=false; for(int i=0;i<300&&!any;++i) any=submit(kPort,mkreq(13,1003,30,40)); CHECK(any);
  ...
  bool done=false; std::uint64_t fgen=0; for(int i=0;i<40000&&!done;++i){ StatusReply s=status(kPort); fgen=s.generated_tokens; if(s.active==0&&s.requests_admitted==admitted) done=true; }
